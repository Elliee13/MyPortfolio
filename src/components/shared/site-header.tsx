import dynamic from "next/dynamic";
import Link from "next/link";

import { SiteHeaderMark } from "@/components/brand/site-header-mark";
import { DesktopNav } from "@/components/shared/desktop-nav";
import { NavItemGitHub } from "@/components/shared/nav-item-github";
import { SiteHeaderWrapper } from "@/components/shared/site-header-wrapper";
import { ToggleTheme } from "@/components/shared/toggle-theme";
import { MAIN_NAV } from "@/config/site";
import { getAllPosts } from "@/features/blog/data/posts";
import { cn } from "@/lib/utils";

const BrandContextMenu = dynamic(() =>
  import("@/components/brand/brand-context-menu").then(
    (mod) => mod.BrandContextMenu
  )
);

const CommandMenu = dynamic(() =>
  import("@/components/shared/command-menu").then((mod) => mod.CommandMenu)
);

const MobileNav = dynamic(() =>
  import("@/components/shared/mobile-nav").then((mod) => mod.MobileNav)
);

export function SiteHeader() {
  const posts = getAllPosts();

  return (
    <SiteHeaderWrapper
      className={cn(
        "sticky top-0 z-50 max-w-screen overflow-x-hidden bg-background px-2 pt-2",
        "data-[affix=true]:shadow-[0_0_16px_0_black]/8 dark:data-[affix=true]:shadow-[0_0_16px_0_black]",
        "not-dark:data-[affix=true]:**:data-header-container:after:bg-border",
        "transition-shadow duration-300"
      )}
    >
      <div
        className="screen-line-before screen-line-after mx-auto flex h-12 items-center justify-between gap-2 border-x border-edge px-2 after:z-1 after:transition-[background-color] sm:gap-4 md:max-w-3xl"
        data-header-container
      >
        <BrandContextMenu>
          <Link
            className="has-data-[visible=false]:pointer-events-none [&_svg]:h-8"
            href="/"
            aria-label="Home"
          >
            <SiteHeaderMark />
          </Link>
        </BrandContextMenu>

        <div className="flex-1" />

        <DesktopNav items={MAIN_NAV} />

        <div className="flex items-center *:first:mr-2">
          <CommandMenu posts={posts} />
          <NavItemGitHub />

          <span className="mx-2 flex h-4 w-px bg-border" />

          <ToggleTheme />
          <MobileNav className="sm:hidden" items={MAIN_NAV} />
        </div>
      </div>
    </SiteHeaderWrapper>
  );
}
